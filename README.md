...
**完整条**——五段数据一线排开，等宽槽位右对齐，数字再跳也不推挤邻段：

<p>
  <img src="docs/shot-bar.png" width="496" alt="完整条"/>
</p>

**小胶囊**——按硬件分组轮播，NET 页与 CPU/GPU 页**精确等宽**，翻页不变形：

<p>
  <img src="docs/shot-capsule.png" width="252" alt="小胶囊"/>
</p>

半透明的玻璃质感小条，永远浮在屏幕右上角。平时它只是一条数据，**鼠标划过去完全无感**——
...
| `NET` | 收 / 发速率（全部网卡求和或单卡） | GetIfTable2 直读字节计数差分（免驱动、免管理员） |
...
<tr><td align="center"><img src="docs/shot-bar.png" width="420"/></td>
<td align="center"><img src="docs/shot-capsule.png" width="210"/></td></tr>
</table>
...
外观 ▸    背景颜色（深色 / 浅色 / 深色毛玻璃 / 浅色毛玻璃）
          透明度（25% / 40% / 55% / 70% / 85% / 100%）
──────────────────────────
显示内容 ▸   CPU 段 / GPU 段 / 内存段 / 硬盘段 / 风扇段 / 网速段
胶囊显示项 ▸  最高温度 / CPU 占用 / CPU 温度 / GPU 占用 / GPU 温度
            显存占用 / 内存占用 / 硬盘温度 / 风扇转速 / 网速
网络 ▸      全部网卡（求和）/ 各网卡选择（● 已连接 / ○ 未连接）
            速率单位（自动 / KB/s / MB/s）
──────────────────────────
...
<details open>
<summary><b>v1.4.0 · 网速监控 + 固定位置修复</b></summary>

**新增**
- 🌐 **网速监控**：GetIfTable2 直读接口字节计数（免驱动、免管理员），收/发速率
  差分计算；完整条与胶囊均可开关，支持全部网卡求和或单卡选择、KB/s / MB/s
  / 自动单位；旧配置升级后自动打开网速（可关，关后不再强制）

**修复**
- 🩹 **固定位置仍会闪动/跳回右上角**：勾选"固定位置"后，10 秒周期的 y 同步
  与避让兜底选位两条路径未被拦截，钉定的胶囊照样被拉回标题栏高度或挪向
  右上角空位。现在固定位置 = x、y 完全尊重用户，不做任何自动移动
- 🩹 **NET 段显示瘦身**：去掉 ↓↑ 箭头与 / 分隔符，改紧凑速率格式（456K /
  88.8M），完整条与胶囊的 NET 宽度大幅收窄；胶囊 NET 页与 CPU/GPU 页
  精确等宽（排版总宽差 <1px，`[capsule-width]` 调试输出可复核）
