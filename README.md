...
|---|---|---|---|---|
| 内存占用 | **~140MB（三进程合计，含 .NET 桥接）** | ~40MB | ~200MB | ~50MB |
| 鼠标穿透 | ✅ 智能穿透 | ❌ | ❌ | 部分 |
| 压住文字自动挪开 | ✅ 内容识别避让 | ❌ | ❌ | ❌ |
| 全屏视频自动隐藏 | ✅ SMTC + 几何判定 | ❌ | ❌ | ❌ |
| 无托盘图标 | ✅ | ❌ | ❌ | ❌ |
...
三进程合计约 140MB 工作集：UI 界面 ~40MB、采集子进程 ~30MB、
桥接进程 ~70MB（.NET 运行时 + LibreHardwareMonitorLib 的大头）。
主程序本体只有 200KB——大头都是生态库。所有进程挂 Job 对象管理，
退出即全部回收，绝不会像某些工具一样在后台悄悄堆积。
</details>
