...
`Rust` · `10MB 内存` · `0.5% CPU` · `鼠标穿透` · `全屏自适应` · `毛玻璃` · `防遮挡避让` · `三进程看门狗`
...
- **零轮询**：全局钩子监听其他窗口的移动 / 显示 / 隐藏 / 前台切换，屏幕没动静就一动不动；
  任何窗口一变化，**1 秒内**完成检测
...
- **事件驱动避让**：WinEvent 全局钩子 + 700ms 节流 + 定时器两段式截屏，
